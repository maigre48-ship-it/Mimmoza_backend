...
function parseCSV(csvText: string): Array<Record<string, string>> {
  ...
  const headers = lines[0].split(",").map(h => h.trim()); const rows: Array<Record<string, string>> = [];
  for (let i = 1; i < lines.length; i++) {
    const values = lines[i].split(","); const row: Record<string, string> = {};
...
async function handleStandard(payload: StandardPayload): Promise<Response> {
  ...
  const output: any = {
    ...
    market_like: {
      ...
      transport: { coverage: transportResult.coverage, score: transportResult.score, label: transportResult.label, summary: transportResult.summary, applicable: transportResult.applicable, mobility_gtfs: transportResult.mobility ?? null }, // v4.4
